...
[![Streamlit App](https://static.streamlit.io/badges/streamlit_badge_black_white.svg)](https://incoherent-rate-model.streamlit.app)

Built with **Streamlit** and **QuTiP** to simulate open quantum system dynamics using state-to-state transition lifetimes ($\tau$) and Lindblad jump operators. This is a **density matrix formalism** solver with custom lifetimes and transitions. You will be requested to define the number of states 'n' (Hilbert space dimension up to 10) in your system, including a ground (sink) state. You can then assign a starting **basis state**/Fock state vector **|m⟩** (this could be the bright state in your system), the time domain for the simulation and rate constants with corresponding state transitions. The time evolution is then simulated with **QuTiP's Lindblad master equation** solver using the **'LSODA'** method. 
...
You can now access this app without any local downloads at https://incoherent-rate-model.streamlit.app or by pressing the Open in Streamlit badge at the top of this document.

## Local Installation Using Git (Self-Hosting):
